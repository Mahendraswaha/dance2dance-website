...
const benefitCards = [
  { key: 'bem_estar', icon: Activity },
  { key: 'conexao', icon: Handshake },
  { key: 'integracao', icon: Users },
  { key: 'presenca', icon: Sun },
  { key: 'criatividade', icon: Zap }
];

const howItWorksIcons = [
  { icon: Compass, key: 'step1' },
  { icon: Heart, key: 'step2' },
  { icon: Sprout, key: 'step3' },
...
export default function CorporateBeTheDancePage() {
  ...
  const body = t('corporate_bethedance.body', { returnObjects: true });
  const benefits = t('corporate_bethedance.benefits', { returnObjects: true });
  const howItWorks = t('corporate_bethedance.how_it_works', { returnObjects: true });
      ...
      <div className="pt-40 md:pt-52 pb-24 relative">
        ...
        <div className="max-w-[900px] mx-auto px-8 md:px-16 lg:px-20 relative z-10">
          ...
          <header className="mb-12">
            ...
            >
              {t('corporate_bethedance.subtitle')}
            </motion.p>
          </header>
        ...
        <section className="mb-24">
          ...
          <div className="max-w-[1100px] mx-auto px-8 md:px-16 lg:px-20">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
              {howItWorksIcons.map(({ icon: Icon, key }, index) => (
                <motion.div
                  key={key}
                  initial="hidden"
                  whileInView="visible"
                  ...
                  variants={fadeUp}
                  custom={index}
                  className="group relative bg-gradient-to-br from-[#141414] to-[#0f0f0f] border border-[#1f1f1f] rounded-[4px] py-10 px-6 md:py-12 md:px-8 flex flex-col items-center justify-center transition-all duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] hover:border-accent/25 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(226,195,102,0.08)] overflow-hidden min-h-[300px]"
                >
                  ...
                  <div className="font-drama text-3xl text-accent mb-2 font-normal opacity-40">0{index + 1}</div>
                  <div className="w-[30px] h-[1px] bg-accent mb-6 opacity-40" />

                  <Icon size={32} className="text-accent/60 group-hover:text-accent transition-colors duration-500 mb-6" strokeWidth={1} />
                  
                  <h4 className="font-heading text-[12px] tracking-[1px] uppercase text-[#E2C366] text-center font-medium leading-[1.4] mb-3">
                    {howItWorks?.items?.[key]?.title}
                  </h4>
                  <p className="font-heading text-[11px] text-[#9A9A9A] font-light leading-[1.6] text-center">
                    {howItWorks?.items?.[key]?.desc}
                  </p>
                </motion.div>
              ))}
            </div>
        ...
        <div className="max-w-[900px] mx-auto px-8 md:px-16 lg:px-20">
          ...
          >
            <p className="font-drama italic text-xl md:text-2xl text-[#E2C366] leading-[1.6] whitespace-pre-line">
              {t('corporate_bethedance.closing')}
            </p>
          </motion.div>
          ...
          >
            <h3 className="font-batang text-2xl text-[#F0EDE8] mb-4 text-center">{t('corporate_bethedance.cta.title')}</h3>
            <p className="font-heading text-[#9A9A9A] font-light text-center mb-8 max-w-lg leading-relaxed">
              {t('corporate_bethedance.cta.text')}
            </p>
            ...
            >
              {t('corporate_bethedance.cta.button')}
            </Link>
